...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2012011361
    //LAB8:EXERCISE2 2012011361  HINT:how to copy the fs in parent's proc_struct?
    /*
...
load_icode(int fd, int argc, char **kargv) {
   ...
   /* (1) create a new mm for current process
	...
	// (6) setup uargc and uargv in user stacks
	//setup argc, argv
	uint32_t argv_size=0, j;
	for (j=0; j<argc; j++)
		argv_size += strnlen(kargv[j],EXEC_MAX_ARG_LEN + 1)+1;
	...
	argv_size = 0;
	for (j=0; j<argc; j++) {
		uargv[j] = strcpy((char *)(stacktop + argv_size ), kargv[j]);
		argv_size +=  strnlen(kargv[j],EXEC_MAX_ARG_LEN + 1)+1;
	}
